...
from modules.process_control import start_sensor, stop_sensor, supervisor_status, start_supervisor, stop_supervisor
...
cache_lock = Lock()
CACHE = {
    "model_df": {"ts": None, "ttl_s": 300, "value": pd.DataFrame()},
    "events": {"ts": None, "ttl_s": 300, "value": []},
    "model_df_larger": {"ts": None, "ttl_s": 900, "value": pd.DataFrame()},
}
...
def show_start_stop_page():
    ...
    This keeps UI consistent with backend after supervisor stop.
    """
    allowed_pretty_names = set(desc_to_key.keys())

    # reconcile each sensor status from heartbeat
...
def supervisor_action():
    ...
    if action == "stop":
        ...
        # IMPORTANT: also disable all sensors and set them Stopped in the UI model
        for pretty, info in models_status.items():
...
def start_stop_agent():
    ...
    if action == "StartAll":
        ...
        for pretty, info in models_status.items():
            ...
            try:
                ...
                models_status[pretty]["status"] = "Running"  # optimistic; GET /startstop will reconcile
                models_status[pretty]["PID"] = None
    ...
    if action.lower() == "start":
        try:
            ...
            start_sensor(SITE_ROOT, sensor_key)
            models_status[agent_name]["status"] = "Running"  # optimistic
