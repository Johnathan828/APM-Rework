...
function fetchLiveUpdates() {
  ...
  fetch(url)
    ...
    .then(data => {
      ...
      let totalAlertProbability = 0;
      let runningModels = models.length;
      let off_sections = 0;
      ...
      Object.keys(groups).sort().forEach(plantSection => {
        ...
        rows.forEach((model, index) => {
          ...
          const barColor =
            (model.latest_result === "No Data" || model.latest_result === "Section Off") ? "grey" :
            (model.latest_result === "Normal") ? "green" :
            (model.latest_result === "Warning") ? "yellow" : "red";
          ...
          totalAlertProbability += (model.alert_probability ?? 0);
      ...
      const averageHealth = runningModels > 0 ? Math.round(totalAlertProbability / runningModels) : 0;
      updateTiles(totalTriggered, averageHealth, runningModels, off_sections, normal_sections);
    })
...
function plotModelHealth(modelHealthData, container, model_details) {
  const xValues = (modelHealthData || []).map(entry => new Date(entry.timestamp).toISOString());
  const modelKey = modelHealthData && modelHealthData[0]
  ...
  const layout = { title: (model_details && model_details.Model_Description) ? model_details.Model_Description : "Model Health" };
...
function plotFeatureContribution(featureData, container, model_details, isHighlighted) {
  ...
  const title = (typeof model_details === 'string')
    ? model_details
    : (model_details && model_details.description ? model_details.description : "Feature");

  const layout = {
    title: { text: title, font: { size: 12 } },
    xaxis: { title: 'Date', type: 'date', tickformat: '%Y-%m-%d %H:%M', hoverformat: '%Y-%m-%d %H:%M', showgrid: true },
    yaxis: (typeof model_details === 'object' && model_details && model_details.unit_description)
  ...
  fetch(url)
    ...
    .then(data => {
      ...
      (data.triggered_tags || []).forEach(tag => {
        ...
        const featDetails = (data.model_details && data.model_details.Features && data.model_details.Features[feat])
      ...
      Object.keys(contribs).forEach(feature => {
        ...
        const featureDic =
          (data.model_details && data.model_details.Features && data.model_details.Features[feature])
            ...
            : (data.model_details && data.model_details.filter_tag ? data.model_details.filter_tag : feature);
